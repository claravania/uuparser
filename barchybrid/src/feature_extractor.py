...
class FeatureExtractor(object):
    ...
    def __init__(self, model, wordsCount, rels, langs, words, ch, nnvecs, options):
        ...
        self.model = model
        if langs:
            self.langs = {lang: ind + 1 for ind,
                          lang in enumerate(langs)}  # +1 for padding vector
        else:
            self.langs = None
        self.nnvecs = nnvecs
        ...
        if self.multiling and not self.shareWordLookup:
            ...
            self.vocab = {}
            for lang in self.langs:
                self.vocab[lang] = {word: ind + 2 for word,
                                    ind in w2i[lang].iteritems()}
        ...
        if not self.multiling or self.shareCharLookup:
            self.chars = {char: ind + 1 for ind,
                          char in enumerate(ch)}  # +1 for OOV vector
        else:
            self.chars = {}
            for lang in self.langs:
                self.chars[lang] = {char: ind + 1 for ind,
                                    char in enumerate(ch[lang])}
        ...
        if not self.multiling or self.shareCharBiLSTM:
            self.char_bilstm = BiLSTM(
                char_in_dims, self.char_lstm_output_size, self.model, dropout_rate=0.33)
        else:
            self.char_bilstms = {}
            for lang in self.langs:
                self.char_bilstms[lang] = BiLSTM(
                    char_in_dims, self.char_lstm_output_size, self.model, dropout_rate=0.33)
        ...
        if not self.multiling or self.shareCharLookup:
            ...
            self.clookups = {}
            for lang in self.langs:
                self.clookups[lang] = self.model.add_lookup_parameters(
                    (len(ch[lang]) + 1, self.char_emb_size))
        ...
        if not self.multiling or self.shareWordLookup:
            ...
            self.wlookups = {}
            for lang in self.langs:
                self.wlookups[lang] = self.model.add_lookup_parameters(
                    (len(words[lang]) + 2, self.word_emb_size))

        if self.multiling and self.lang_emb_size > 0:
            self.langslookup = model.add_lookup_parameters(
                (len(langs) + 1, self.lang_emb_size))

        """Padding"""
        self.word2lstm = model.add_parameters(
    ...
    def get_char_vec(self, word, dropout, lang=None, langvec=None):
        if word.form == "*root*":
            # use the padding vector if it's the word token
            word.chVec = self.chPadding.expr()
        else:
            char_vecs = []
            ...
            if lang:
                word.chVec = self.char_bilstms[
                    lang].get_sequence_vector(char_vecs, dropout)
            else:
                word.chVec = self.char_bilstm.get_sequence_vector(
                    char_vecs, dropout)
            ...
            for lang in self.langs:
                ...
                self.paddingVecs[lang] = dy.tanh(self.word2lstm.expr() * dy.concatenate(filter(None,
                                                                                               [paddingWordVecs[lang],
                                                                                                ...
                                                                                                paddingLangVec if self.word_lembed else None]))
                                                 + self.word2lstmbias.expr())
                self.emptyVecs[lang] = self.paddingVecs[lang] if self.nnvecs == 1 else dy.concatenate(
    ...
    def getWordEmbeddings(self, sentence, train):
        ...
        for root in sentence:
            ...
            noDropFlag = not train or (
                random.random() < (wordcount / (0.25 + wordcount)))
            if not self.multiling or self.shareWordLookup:
                root.wordvec = self.wlookup[
                    int(self.vocab.get(root.norm, 0)) if noDropFlag else 0]
            else:
                root.wordvec = self.wlookups[lang][
                    int(self.vocab[lang].get(root.norm, 0)) if noDropFlag else 0]
    ...
    def get_external_embeddings(self, external_embedding_file, model):
        ...
        external_embedding_fp = codecs.open(
            external_embedding_file, 'r', encoding='utf-8')
        ...
        self.elookup = model.add_lookup_parameters(
