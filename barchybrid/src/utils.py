    ...
    def __init__(self, treebank_info, location, shared_task=False, shared_task_data_dir=None):
        ...
        if shared_task:
            ...
            if treebank_info['tcode'] == '0':
                ...
                self.iso_id = treebank_info[
                    'lcode'] + '_' + treebank_info['tcode']
...
def isProj(sentence):
    ...
    unassigned = {entry.id: sum(
        [1 for pentry in sentence if pentry.parent_id == entry.id]) for entry in sentence}
...
def vocab(conll_path, path_is_dir=False, shareWordDict=True, shareCharDict=True):
    ...
    for sentence in data:
        ...
        # TODO:this is hacky to do that at every word - should do it at every
        # new lang
        if path_is_dir:
        ...
        if not tok or line.strip() == '':
            if len(tokens) > 1:
                ...
                if not drop_nproj or isProj(conll_tokens):
                    ...
                    # if not isProj([t for t in tokens if isinstance(t,
                    # ConllEntry)]):
                    ...
                    for tok in conll_tokens:
                        tok.rdeps = [
                            i.id for i in conll_tokens if i.parent_id == tok.id]
                        if tok.id != 0:
                            tok.parent_entry = [
                                i for i in conll_tokens if i.id == tok.parent_id][0]
                    if maxSize > 0:
                        if not hard_lim:
            ...
            if line[0] == '#' or '-' in tok[0] or '.' in tok[0]:
                ...
                token = ConllEntry(int(tok[0]), tok[1], tok[2], tok[4], tok[3], tok[5], int(
...
def evaluate(gold, test, conllu):
    ...
    if not conllu:
        ...
        os.system('perl src/utils/eval.pl -g ' + gold +
                  ' -s ' + test + ' > ' + scoresfile)
    else:
...
def inorder(sentence):
    ...
    def inorder_helper(sentence, i):
        results = []
        left_children = [entry for entry in sentence[
            :i] if entry.parent_id == i]
        for child in left_children:
            results += inorder_helper(sentence, child.id)
        results.append(sentence[i])

        right_children = [entry for entry in sentence[
            i:] if entry.parent_id == i]
