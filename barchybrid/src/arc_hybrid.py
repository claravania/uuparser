...
class ArcHybridLSTM:
    ...
    def __init__(self, words, pos, rels, cpos, langs, w2i, ch, options):
        ...
        # import here so we don't load Dynet if just running parser.py --help
        # for example
        import dynet as dy
        global dy
        ...
        self.nnvecs = (1 if self.headFlag else 0) + \
            (2 if self.rlFlag or self.rlMostFlag else 0)
        self.feature_extractor = FeatureExtractor(
            self.model, words, rels, langs, w2i, ch, self.nnvecs, options)
        ...
        if not options.multiling or self.shareMLP:
            self.unlabeled_MLP = MLP(
                self.model, mlp_in_dims, h1, h2, 4, self.activation)
            self.labeled_MLP = MLP(
                self.model, mlp_in_dims, h1, h2, 2 * len(rels) + 2, self.activation)
        else:
    ...
    def __evaluate(self, stack, buf, train):
        ...
        topStack = [
            stack.roots[-i - 1].lstms if len(stack) > i else [empty] for i in xrange(self.k)]
        topBuffer = [buf.roots[i].lstms if len(
            buf) > i else [empty] for i in xrange(1)]
        ...
        if train:
            ...
            ret = [[(rel, 0, scrs[2 + j * 2] + uscrs2, routput[2 + j * 2] + output2) for j, rel in enumerate(self.irels)] if left_arc_conditions else [],
                   [(rel, 1, scrs[3 + j * 2] + uscrs3, routput[3 + j * 2] + output3)
                    for j, rel in enumerate(self.irels)] if right_arc_conditions else [],
                   [(None, 2, scrs[0] + uscrs0, routput[0] + output0)
    ...
    def apply_transition(self, best, stack, buf, hoffset):
        ...
        if best[1] == 0 or best[1] == 1:
            ...
            if self.rlMostFlag:
                parent.lstms[
                    best[1] + hoffset] = child.lstms[best[1] + hoffset]
    ...
    def calculate_cost(self, scores, s0, s1, b, beta, stack_ids):
        if len(scores[0]) == 0:
            ...
            left_cost = len(s0[0].rdeps) + int(s0[0].parent_id !=
                                               b[0].id and s0[0].id in s0[0].parent_entry.rdeps)

        if len(scores[1]) == 0:
            right_cost = 1
        else:
            right_cost = len(s0[0].rdeps) + int(s0[0].parent_id !=
                                                s1[0].id and s0[0].id in s0[0].parent_entry.rdeps)
        ...
        if len(scores[2]) == 0:
            ...
            shift_cost = len([d for d in b[0].rdeps if d in stack_ids]) + int(len(s0) > 0 and b[
                0].parent_id in stack_ids[:-1] and b[0].id in b[0].parent_entry.rdeps)
            shift_case = 2
    ...
    def Predict(self, data):
        ...
        for iSentence, osentence in enumerate(data, 1):
            ...
            for root in conll_sentence:
                ...
                if not self.feature_extractor.multiling or self.feature_extractor.shareWordLookup:
                    root.lstms += [self.feature_extractor.paddingVec for _ in range(
                        self.nnvecs - hoffset)]
                else:
                    root.lstms += [self.feature_extractor.paddingVecs[lang]
                                   for _ in range(self.nnvecs - hoffset)]

            while not (len(buf) == 1 and len(stack) == 0):
                scores = self.__evaluate(stack, buf, False)
                best = max(
                    chain(*(scores if iSwap < max_swap else scores[:3])), key=itemgetter(2))
            ...
            oconll_sentence = [
                entry for entry in osentence if isinstance(entry, utils.ConllEntry)]
    ...
    def Train(self, trainData):
        ...
        # in certain cases the data will already have been shuffled after being
        # read from file or while creating dev data
        ...
        for iSentence, sentence in enumerate(trainData, 1):
            ...
            # ensures we are working with a clean copy of sentence and allows
            # memory to be recycled each time round the loop
            ...
            while not (len(buf) == 1 and len(stack) == 0):
                ...
                bestValid = list((s for s in chain(
                    *scores) if costs[s[1]] == 0 and (s[1] == 2 or s[1] == 3 or s[0] == s0[0].relation)))
                if len(bestValid) < 1:
                ...
                bestWrong = max((s for s in chain(*scores) if costs[s[1]] != 0 or (
                    s[1] != 2 and s[1] != 3 and s[0] != s0[0].relation)), key=itemgetter(2))
                ...
                if best[1] == 2:
                    ...
                    if shift_case == 2:
                        ...
                        blocked_deps = [d for d in b[
                            0].rdeps if d in stack_ids]
